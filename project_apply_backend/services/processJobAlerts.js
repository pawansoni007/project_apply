...
async function processJobAlerts(userId) {
  const mostRecentJobs = await fetchAndStoreJobPosts();
  if (!mostRecentJobs || mostRecentJobs.length === 0) return;

  const user = await User.findById(userId);

  let newJobPosts;

  if (user.lastProcessedJobId === '') {
    newJobPosts = mostRecentJobs;
  } else {
    const lastProcessedIndex = mostRecentJobs.findIndex(
      (job) => job.id === user.lastProcessedJobId
    );
    newJobPosts =
      lastProcessedIndex === -1
        ? mostRecentJobs
        : mostRecentJobs.slice(0, lastProcessedIndex);
  }
  ...
  await processUnextractedJobPosts(parsedJobPostsWithClampedJsons.slice(0, 10)); // TODO - gotta change this to process all jobs

  const jobIds = parsedJobPostsWithClampedJsons.map((job) => job.id);

  const jobPostsToFilterForUser = await JobPost.find({
    id: { $in: jobIds },
  }).select(
  ...
  logger.info(
    `jobPostsToFilterForUser: ${JSON.stringify(jobPostsToFilterForUser)}`
  );

  logger.info(`jobPostsToFilterForUser: ${jobPostsToFilterForUser.length}`);

  parsedJobPostsWithClampedJsons = null;
  ...
  if (jobPostsToFilterForUser.length > 0) {
    ...
    const filteredJobs = await filterJobsWithGemini(
      user.jobFilter,
      jobPostsToFilterForUser.slice(0, 10), // TODO - gotta change this to filter all jobs
      resumeFile
    );
    ...
    // user.lastProcessedJobId = mostRecentJobs[0].id;
    // await user.save();
  }
  return {
    message: 'Job alerts processed successfully',
    // jobsProcessed: mostRecentJobs.length,
    // newJobsFound: newJobPosts.length,
    // filteredJobsCount: filteredJobs ? filteredJobs.length : 0
  };
}

async function processUnextractedJobPosts(parsedJobPostsWithClampedJsons) {
  let unprocessedJobPosts = await Promise.all(
    parsedJobPostsWithClampedJsons.map(async (job) => {
      const exists = await JobPost.exists({
        id: job.id,
  ...
  unprocessedJobPosts = unprocessedJobPosts.filter((job) => job !== null);
