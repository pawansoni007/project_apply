...
async function extractJobInfo(parsedJobPostsWithClampedJsons) {
  ...
  - Do not include any escape characters in the output.
  - Double check if the JSON is valid before returning
...
`;
  ...
  try {
    ...
    // logger.info(
    //   `Extracted job info: ${JSON.stringify(parsedJobInfo, null, 2)}`
    // );
